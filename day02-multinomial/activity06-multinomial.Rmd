...
Maybe they wanted to get a good sample size to have some data to refer to - chances when they did or did not decide to vote.
...
```{r}
ggplot(nonvoters, aes(x = ppage, y = voter_category)) + 
  geom_count()

ggplot(nonvoters, aes(x = race, y = voter_category)) + 
  geom_count()
```
...
```{r create party}
#create new variable coding each person as one of Rep, Dem, Indep, or Other based on response to Q30.
...
#relevel this var as a factor variable w/ rarely/never as reference level
nonvoters$voter_category <- factor(nonvoters$voter_category, ordered = TRUE, levels = c("rarely/never", "sporadic", "always"))
#note that the order defines the reference level (first) and subsequent order 
...
```{r}
ggplot(nonvoters, aes(x = voter_category, fill = party, y = party)) + 
