...
- Give your R code chunk a meaningful name, then run your code chunk or knit your document.
```{r load data}
#use data in data folder and read_csv to create data frame 
resume <- read_csv("data/resume.csv")
#check out some summary info
glimpse(resume)
skimr::skim(resume)
...
I would call this an experiment. They designed the resumes and tested their results rather than looking back on some data retrospectively. This includes changing and manipulating some factors (race and gender of names) and testing outcomes.

2. The variable of interest is `received_callback`.
  What type of variable is this?
  What do the values represent?

It is a binary categorical variable where the status of whether a resume received a callback is recorded. 1 means yes, 0 means no. 
...
ggplot(resume, 
  ...
  geom_bar() + 
  labs(title = "received_callback outcomes",
       x = "received_callback",
       y = "count") + 
  theme_minimal()
```
...
resume %>% 
  ...
  )) %>% #mutating data so that values become No and Yes according to binary coding
  count(received_callback) %>% #count each category
  mutate(percent = round(n / sum(n) * 100, 2)) %>% #create percentages for each
...
There is a low likelihood of receiving a callback at all (about 8% of the time) for these data.  
...
.08/1-.08 = About 8.7%
```{r}
#calculating the odds of a callback for random person
.08/(1-.08 )
```
...
Y-hat = (e^(-2.675 + .438(raceWhite)))/1 + e^(-2.675 + .438(raceWhite))
...
6.45%.(calculated by y hat above)
...
It limits it to chicago jobs and changes some labels for gender and race. 
...
There are more females than males, there are no other obvious patterns. 
...
The odds of a callback increase by 4.5% holding all else constant.
...
mult_log_aug <- augment(resume_mod, type.predict = "response", 
